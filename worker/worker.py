...
class DemandForecastingService:
    def __init__(self, forecast_db_uri: str, forecast_db_name: str,
                 transactions_db_uri: str, transactions_db_name: str):
        self.forecast_db = MongoDBConnectionManager(forecast_db_uri, forecast_db_name)
        self.transactions_db = MongoDBConnectionManager(transactions_db_uri, transactions_db_name)

    def load_transaction_data(self, partner_id: str) -> pd.DataFrame:
    ...
    def update_forecast(self, forecast_id: str, prediction: dict):
        collection = self.forecast_db.get_collection("demand_forecast")
...
class ForecastingWorker:
    def __init__(self, rabbitmq_host: str, rabbitmq_user: str, rabbitmq_password: str, queue: str,
                 forecast_db_uri: str, forecast_db_name: str,
                 transactions_db_uri: str, transactions_db_name: str):
        ...
        self.forecasting_service = DemandForecastingService(
            forecast_db_uri, forecast_db_name,
            transactions_db_uri, transactions_db_name
        )
    ...
    def perform_forecasting(self, data: dict):
        ...
        try:
            ...
            df = self.forecasting_service.load_transaction_data(partner_id)
            if df.empty:
...
def main():
    ...
    rabbitmq_user = os.getenv("RABBITMQ_USERNAME", "aiselena")
    rabbitmq_password = os.getenv("RABBITMQ_PASSWORD", "s4cret")

    # Separate URIs for different databases
    forecast_db_uri = os.getenv("FORECAST_DB_URI", "mongodb+srv://vithchatayasaharat:4gDwprHyrAQeX3!@cluster0.hmyqrhd.mongodb.net")
    transactions_db_uri = os.getenv("TRANSACTIONS_DB_URI", "mongodb://root:example@89.213.177.102:27017/")

    forecast_db_name = os.getenv("FORECAST_DB_NAME", "aiselena")
    transactions_db_name = os.getenv("TRANSACTIONS_DB_NAME", "aiselena_logistic")
